package com.store.layouts
{
	...
	public class FilteredTileLayout extends LayoutBase
	{
		...
		override public function measure():void {
			...
			var width   : Number     = (_target.width - 2*_padding);
			var numCols : int        = width / Math.floor((_tileWidth+_horizontalGap)*_scaling);
			var numRows : int        = Math.ceil(target.numElements/numCols);
			
			_target.measuredWidth  = numCols * (_tileWidth+_horizontalGap)*_scaling - _horizontalGap;
			_target.measuredHeight = numRows * (_tileHeight+_verticalGap)*_scaling - _verticalGap;
		}
